...
class Settings:
    ...
    mlflow_experiment_name: str = os.getenv(
        "MLFLOW_EXPERIMENT_NAME", "medassist-qlora-sft"
    )
    model_registry_name: str = os.getenv(
        "MODEL_REGISTRY_NAME", "medassist-medical-lora"
    )
    ...
    enable_query_decomposition: bool = (
        os.getenv("ENABLE_QUERY_DECOMP", "true").lower() == "true"
    )
    enable_self_rag: bool = os.getenv("ENABLE_SELF_RAG", "true").lower() == "true"
    self_rag_confidence_floor: float = float(
        os.getenv("SELF_RAG_CONFIDENCE_FLOOR", "0.4")
    )
