...
def estimate(
    ...
    gpu_hourly_usd: float = None,
    throughput_req_per_hour: float = None,
    replicas: int = 1,
) -> dict:
    pricing = INSTANCE_PRICING[cloud][model_size]
    hourly = gpu_hourly_usd if gpu_hourly_usd is not None else pricing["hourly_usd"]
    throughput = (
        throughput_req_per_hour
        if throughput_req_per_hour is not None
        else DEFAULT_THROUGHPUT[model_size]
    )
    gateway_hourly = INSTANCE_PRICING[cloud]["gateway"]["hourly_usd"]

    per_hour_capacity = throughput * replicas
    hours_per_day = (
        requests_per_day / per_hour_capacity if per_hour_capacity else float("inf")
    )
    gpu_cost_per_day = (
        hourly * replicas * min(24.0, hours_per_day if hours_per_day else 0)
    )
    ...
    hours_running = (
        min(24.0, requests_per_day / per_hour_capacity) if per_hour_capacity else 24.0
    )
    gpu_cost_per_day = hourly * replicas * hours_running
    gateway_cost_per_day = (
        gateway_hourly * 24
    )  # gateway assumed always-on; scale-to-zero saves this
    ...
    return {
        ...
        "note": (
            "If requests_per_day exceeds a single replica's daily capacity "
...
def main():
    ...
    print(
        f"MedAssist-GenAI cost estimate -- {result['cloud'].upper()}, {result['model_size']} model"
    )
    print("-" * 60)
    print(f"  Instance:                 {result['instance']} x{result['replicas']}")
    print(f"  GPU price:                ${result['gpu_hourly_usd']}/hr per instance")
    print(
        f"  Assumed throughput:       {result['assumed_throughput_req_per_hour_per_replica']} req/hr/replica"
    )
